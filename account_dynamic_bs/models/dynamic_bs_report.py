...
class ReportDynamicFinancialBs(models.AbstractModel):
    _name = 'report.account.dynamic.report_financial_bs'
    ...
    def get_account_lines(self, data):
        ...
        account_report = self.env.ref('account.account_financial_report_balancesheet0')
    ...
    def get_report_values(self, docids, data=None):
        ...
        if data['form'].get('date_filter'):
            ...
            date_format = '%Y-%m-%d'
            langs = self.env['res.lang'].search([('active', '=', True)])[0]
            if langs:
                date_format = langs.date_format
            used_context['date_from'] = datetime.strptime(data['form'].get('date_from', False),date_format).strftime('%Y-%m-%d')
            used_context['date_to'] = datetime.strptime(data['form'].get('date_to', False),date_format).strftime('%Y-%m-%d')
    ...
    def action_pdf(self, docids, data=None):
        data, report_lines = self.get_report_values(docids, data)
        data['form']['account_report_id'] = self.env.ref('account.account_financial_report_balancesheet0').id
