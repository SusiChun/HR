        ...
        */
        find_child: function (el, line, line_type, self) {
            ...
            if (line_type != 'journal_item' && line_type != 'account'){
                ...
                if(current_line){
//                Changed by milind category to journal_items
...
//            Changed by milind category to journal_items
        ...
        create_lines_with_style: function(rec, attr){
            ...
            if(rec['type'] == 'journal_item'){
                ...
                //add name
                temp_str += "<td class='child_col_name'  "+attr_name+" >"+rec['date'] + "</td>";
                temp_str += "<td class='child_col_bal' "+attr_bal+" >"+rec['name'] + "</td>";
                temp_str += "<td class='child_col_bal' "+attr_bal+" >"+rec['partner_name'] + "</td>";
                temp_str += "<td class='child_col_bal' "+attr_bal+" >"+rec['description'] +  "</td>";
                //add balance and debit credit colums
                if (this.form.debit_credit == true) {
                    // debit

                    temp_str += "<td class='child_col_bal' "+attr_bal+" >"+
                        this.currency + " " + rec['debit'].toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')+
                        "</td>";
                    // credit
                    temp_str += "<td class='child_col_bal' "+attr_bal+" >"+
                        this.currency + " " + rec['credit'].toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')+
                        "</td>";
                    // balance
                    temp_str += "<td class='child_col_bal' "+attr_bal+" >"+
                        this.currency + " " + rec['balance'].toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')+"</td></tr>";
                }
                else {
                    temp_str += "<td class='child_col_bal' "+attr+" >"+
                        this.currency + " " + rec['balance'].toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')+
        ...
        renderElement: function () {
            ...
            for (var c in first_childs) {
                first_childs[c]['balance_r'] =
                    first_childs[c]['balance'] ? first_childs[c]['balance'].toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,'): 0.00;
                    // first_childs[c]['balance'] ? first_childs[c]['balance'].toFixed(2): 0.00;
                first_childs[c]['debit_r'] =
                    first_childs[c]['debit'] ? first_childs[c]['debit'].toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,'): 0.00;
                first_childs[c]['credit_r'] =
                    first_childs[c]['credit'] ? first_childs[c]['credit'].toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,'): 0.00;
            }
