        ...
        for report in child_reports:
            ...
            if res[report.id].get('account'):
                ...
                for account_id, value\
                    ...
                    vals = {
                        'account': account.id,
                        # Changed by milind added string acnt
            ...
            if i['type'] == 'account':
                ...
                if form['target_move'] == 'posted':
                    search_query = "select aml.id, am.id as j_id, aml.account_id, aml.date," \
                        ...
                        + "(aml.debit-aml.credit) as balance, aml.debit, aml.credit, aml.partner_id,rp.name as partner_name, aml.name as description " \
                        + " from account_move_line aml join account_move am " \
                          "on (aml.move_id=am.id and am.state=%s)  join res_partner rp on (aml.partner_id=rp.id)" \
                    ...
                    search_query = "select aml.id, am.id as j_id, aml.account_id, aml.date, " \
                                   "aml.name as label, am.name, " \
                        + "(aml.debit-aml.credit) as balance, aml.debit, aml.credit, aml.partner_id,rp.name as partner_name, aml.name as description" \
                        + " from account_move_line aml join account_move am on (aml.move_id=am.id) join res_partner rp on (aml.partner_id=rp.id) " \
                        + " where aml.account_id=%s"
                    vals = []
