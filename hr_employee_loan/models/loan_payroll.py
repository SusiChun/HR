...
#    )
    ...
    total_pinjaman        = fields.Float(required=True,readonly=True,compute='compute_limit_for_wage')
...
#         return self.write({'state':'approve'})
    ...
    def compute_limit_for_wage(self):
        for x in self:
            data     = self.env['employee.loan.details'].search([('name', '=', self.name)])
            for y in data:
                if y:
                    self.total_pinjaman = y.principal_amount


    def action_hitung(self):
        self.env.cr.execute("""
                                SELECT
                                    SUM(a.principal_amt) as total_loan, 
                                    COUNT(a.*) as max
                                FROM
                                    loan_installment_details a 
                                WHERE
                                    a.name = %s
                                """,(self.name,))

        DTLoan          = self.env.cr.dictfetchone()        
        SumTot_loan     = DTLoan['total_loan']
        MaxDT           = DTLoan['max']
        
        user_id         = self.env.user.id
        pinjamanTot     = self.total_pinjaman
        # print '======================================================== total loan', pinjamanTot
        # print '======================================================== total bayar', SumTot_loan
        # print '======================================================== MAX', int(MaxDT)+1
        max_no = int(MaxDT)+1
        intall = int(MaxDT)
        if pinjamanTot > SumTot_loan:
            nilai_sisa = pinjamanTot - SumTot_loan
            int_payable ='f'
            curency_id='13'
            print '======================================================== Sisa <', nilai_sisa
            # insert new line
            state = 'unpaid'
            self.total = self.principal_amt
            self.env.cr.execute(""" UPDATE employee_loan_details SET duration = %s WHERE id = %s""", (max_no, self.loan_id.id,))
            self.env.cr.execute(""" INSERT INTO loan_installment_details (loan_id,    currency_id,  principal_amt,      loan_state,         total,      create_uid,             employee_id,            loan_type,          int_payable,    company_id,         state,      name,       interest_amt,       install_no) VALUES (%s,              %s,           %s,                 %s,                 %s,         %s,                     %s,                     %s,                 %s,             %s,                 %s,         %s,         %s,                 %s)""", (self.loan_id.id, curency_id,   nilai_sisa, self.loan_state ,   nilai_sisa, user_id,    self.employee_id.id,    self.loan_type.id,  int_payable,    self.company_id.id, state, self.name,  self.interest_amt,  max_no))    
            # self.env.cr.execute(""" INSERT INTO loan_installment_details (loan_id,    currency_id,  principal_amt,      loan_state,         total,      create_uid,             employee_id,            loan_type,          int_payable,    company_id,         state,      name,       interest_amt,       install_no) VALUES (%s,              %s,           %s,                 %s,                 %s,         %s,                     %s,                     %s,                 %s,             %s,                 %s,         %s,         %s,                 %s)""", (self.loan_id.id, curency_id,   self.principal_amt, self.loan_state ,   nilai_sisa, user_id,    self.employee_id.id,    self.loan_type.id,  int_payable,    self.company_id.id, state, self.name,  self.interest_amt,  max_no))    
            self.env.cr.commit()
        else:
            nilai_sisa = SumTot_loan - pinjamanTot
            print '+++++++++++++++++++++++ Nilai Sisa',nilai_sisa

            self.total = self.principal_amt
            data = self.env[('loan.installment.details')].search([('loan_id','=',self.loan_id.id),('state','=','unpaid')],order='id desc')
            for z in data:
                print '+++++++++++++++', nilai_sisa
                if nilai_sisa >= z.total:
                    nilai_sisa = nilai_sisa - z.total
                    print 'update total = 0, sisa lanjut next row',nilai_sisa
                    print '============= ID Detail', z.id
                    print '============= Total', z.total
                    print '============= Sisa untuk di hitung di next row', nilai_sisa
                    self.env.cr.execute(""" UPDATE loan_installment_details SET total = 0, principal_amt = 0, state = 'cancel' WHERE id = %s""", (z.id,))
                else:
                    nilai_sisa = z.total - nilai_sisa
                    print 'update total = nilai sisa dan stop proses',nilai_sisa
                    print '============= ID Detail', z.id
                    print '============= Total', z.total
                    self.env.cr.execute(""" UPDATE loan_installment_details SET total = %s, principal_amt = %s, state = 'unpaid' WHERE id = %s""", (nilai_sisa,nilai_sisa, z.id,))
                    return

                nilai_sisa = nilai_sisa
             # self._cr.execute(""" DELETE FROM loan_installment_details WHERE total = 0""",())
