...
class AdvanceExpenseLine(models.Model):
    ...
    # Added by Baim
    @api.multi
    @api.depends('unit_amount','quantity','total_amount','state')
    def _compute_total_last_balance(self):
        for rec in self:
            total = 0.0
            for line in rec.expense_line_ids:
                total += line.total_amount
            rec.last_balance = rec.advance_line_id.total_amount_expense - total
    ...
    expense_line_ids = fields.One2many('hr.expense', 'advance_expense_id', string='Expense Lines', copy=False)
