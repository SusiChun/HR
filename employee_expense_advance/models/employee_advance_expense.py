...
class EmployeeAdvanceExpense(models.Model):
    ...
    # Added by Baim
    @api.multi
    @api.depends('advance_expense_line_ids', 'advance_expense_line_ids.last_balance')
    def _compute_total_last_balance(self):
        for rec in self:
            rec.last_balance = 0.0
            for line in rec.advance_expense_line_ids:
                rec.last_balance += line.last_balance
    ...
    last_balance = fields.Float(string='Last Balance', compute='_compute_total_last_balance', store=False, digits=dp.get_precision('Account'))
    ...
    journal_return_id = fields.Many2one('account.journal', string='Payment Return')
    ...
    # Added by Baim
    @api.multi
    def action_sheet_move_advance_return(self):
        created_moves = self.env['account.move']
        prec = self.env['decimal.precision'].precision_get('Account')
        if not self.account_id:
            raise UserError(_("No Debit account found for the Account, please configure one.") % (self.account_id))
        if not self.journal_return_id:
            raise UserError(_("No Credit account found for the Journal, please configure one.") % (self.journal_return_id))
        for line in self:
            # category_id = line.asset_id.category_id
            adv_exp_date = fields.Date.context_today(self)
            company_currency = line.company_id.currency_id
            current_currency = line.currency_id
            amount = current_currency.compute(line.last_balance, company_currency)
            ref = line.name 
            move_line_credit = {
                'name': ref,
                'account_id': line.journal_return_id.default_credit_account_id.id,
                'debit': 0.0 if float_compare(amount, 0.0, precision_digits=prec) > 0 else -amount,
                'credit': amount if float_compare(amount, 0.0, precision_digits=prec) > 0 else 0.0,
                'journal_id': line.journal_return_id.id,
                'partner_id': line.partner_id.id,
                # 'analytic_account_id': category_id.account_analytic_id.id if category_id.type == 'sale' else False,
                'currency_id': company_currency != current_currency and current_currency.id or False,
                'amount_currency': company_currency != current_currency and - 1.0 * line.last_balance or 0.0,
            }
            move_line_debit = {
                'name': ref,
                'account_id': line.account_id.id,
                ...
                # 'analytic_account_id': category_id.account_analytic_id.id if category_id.type == 'purchase' else False,
                'currency_id': company_currency != current_currency and current_currency.id or False,
                'amount_currency': company_currency != current_currency and line.last_balance or 0.0,
            }
            move_vals = {
                'ref': line.name,
                'date': adv_exp_date or False,
