...
class AttendanceReport(models.TransientModel):
    ...
    def print_report_excel(self):
        ...
        title_style1_table_head_bold_leave = xlwt.easyxf('borders: left medium, right medium, top medium, bottom medium;align: horiz center ;font: name Times New Roman,bold on, italic off,height 200,color blue;')
        ...
        if self.attendance_details_duplicate:
            ...
            for each in details:
               for rec in each.get('data'):
                   ...
                   for val in rec['value']:
                       if val.get('state'):
                           for le in val['state']:
                               ...
                               if val['state'] == 'L':
                                    sheet.write(row_data,cl_data,le,title_style1_table_head_bold_leave)
                               if val['state'] == 'H':
                                    sheet.write(row_data,cl_data,le,title_style1_table_head_bold_leave)
                               if val['state'] == '-':
                                   sheet.write(row_data, cl_data, le, title_style1_table_head_bold_absent)
                   ...
                   sheet.write(row_data, cl_data + 2, str(val['leave']), title_style1_table_head_bold)
                   sheet.write(row_data, cl_data + 3, str(val['public_holiday']), title_style1_table_head_bold)
